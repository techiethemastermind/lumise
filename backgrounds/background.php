<?php
	...
	$fields = $lumise_admin->process_data(array(
		...
		array(
			'type' => 'upload',
			'name' => 'upload',
			'path' => 'backgrounds'.DS.date('Y').DS.date('m').DS,
	...
	<div class="lumise_content">
		<?php
			$lumise->views->detail_header(array(
				'add' => '<i class="fa fa-plus"></i> '.$lumise->lang('Add New Background'),
				'edit' => $lumise->lang('Edit Background'),
				'page' => $section
			));
		?>
		<form action="<?php echo $lumise->cfg->admin_url; ?>lumise-page=<?php
			echo $section.(isset($_GET['callback']) ? '&callback='.$_GET['callback'] : '');
		?>" id="lumise-background-form" method="post" class="lumise_form" enctype="multipart/form-data">

			<?php $lumise->views->tabs_render($fields); ?>

			<div class="lumise_form_group lumise_form_submit">
				<input type="submit" value="<?php echo $lumise->lang('Save Background'); ?>"/>
				<input type="hidden" name="do" value="action" />
				<a class="lumise_cancel" href="<?php echo $lumise->cfg->admin_url;?>lumise-page=backgrounds">
