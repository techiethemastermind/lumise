<?php
	...
	if (isset($_POST['action_submit']) && !empty($_POST['action_submit'])) {
		...
		$lumise_admin->check_caps('distresss');
		
		foreach ($val as $value) {

			$dt = $lumise_admin->get_row_id($value, 'distress');
			switch ($data_action) {
				...
				case 'active':
					...
					$dt = $lumise_admin->edit_row( $value, $data, 'distress' );
					break;
				case 'deactive':
					$data = array(
						'active' => 0
				...
				case 'delete':
					...
					$lumise_admin->delete_row($value, 'distress');
	...
	// Search Form
	$data_search = '';
	if (isset($_POST['search_distress']) && !empty($_POST['search_distress'])) {
	...
	if (isset($_POST['do']) && !empty($_POST['do'])) {
		$lumise->redirect($lumise->cfg->admin_url . "lumise-page=distressings");
		exit;
	}
    ...
    $select = "SELECT SQL_CALC_FOUND_ROWS art.* FROM {$lumise->db->prefix}distress art ";
	...
	$total_record = $lumise->db->rawQuery("SELECT count(*) as total FROM {$lumise->db->prefix}distress");
	...
	$config = array(
		...
		'total_page'    => ceil($total_record[0]['total']/$per_page),
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'lumise-page=distressings&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'lumise-page=distressings',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			<h2><?php echo $lumise->lang('Distresses'); ?></h2>
			<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=distressing" class="add_new">
				<i class="fa fa-plus"></i>
				<?php echo $lumise->lang('Add New Distress'); ?>
			</a>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=distressings" method="post">
				...
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=distressings" method="post" class="less">
			...
			<div class="right">
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=distressings" method="post" class="less">
					...
					<input type="hidden" name="search_distress" value="<?php echo $lumise->lang('Search'); ?>">
					<?php $lumise->securityFrom();?>
				</form>
			</div>
		</div>
							...
							<tr>
								...
								<td>
									<?php
										...
										$dt = $lumise_admin->get_category_item($art['id'], 'distress');
										$dt_name = array();
								...
								<td style="width:20%; position:relative;">
									<?php
										$art['id'] = isset($art['id']) ? $art['id'] : '';
										$dt = $lumise_admin->get_tag_item($art['id'], 'distress');
										$dt_name = array();
										foreach ($dt as $val) {
											$dt_name[] = $val['name'];
										}
									?>
									<input name="tags" class="tagsfield" value="<?php echo implode(',', $dt_name); ?>" data-id="<?php echo $art['id']; ?>" data-type="distress">
								</td>
								<td>
									<?php
								...
								<td>
									<a href="#" class="lumise_action" data-type="distress" data-action="switch_active" data-status="<?php echo (isset($art['active']) ? $art['active'] : '0'); ?>" data-id="<?php if(isset($art['id'])) echo $art['id'] ?>">
										<?php
											if (isset($art['active'])) {
												if ($art['active'] == 1) {
		...
		<?php } else {
					...
					else echo '<p class="no-data">'.$lumise->lang('No data. Please add distress.').'</p>';
			}?>
