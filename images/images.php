<?php
	...
	if (isset($_POST['action_submit']) && !empty($_POST['action_submit'])) {
		...
		$lumise_admin->check_caps($sects);
		
		foreach ($val as $value) {

			$dt = $lumise_admin->get_row_id($value, $sects);
			switch ($data_action) {
				...
				case 'active':
					...
					$dt = $lumise_admin->edit_row( $value, $data, $sects);
					break;
				case 'deactive':
					$data = array(
						'active' => 0
				...
				case 'delete':
					...
					$lumise_admin->delete_row($value, $sects);
	...
	// Search Form
	$data_search = '';
	if (isset($_POST['search_'.$sect]) && !empty($_POST['search_'.$sect])) {
	...
	if (isset($_POST['do']) && !empty($_POST['do'])) {
		$lumise->redirect($lumise->cfg->admin_url . "lumise-page=".$sects);
		exit;
	}
    ...
    $select = "SELECT SQL_CALC_FOUND_ROWS art.* FROM {$lumise->db->prefix}$sects art ";
	...
	$total_record = $lumise->db->rawQuery("SELECT count(*) as total FROM {$lumise->db->prefix}$sects");
	...
	$config = array(
		...
		'total_page'    => ceil($total_record[0]['total']/$per_page),
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'lumise-page='.$sects.'&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'lumise-page='.$sects,
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			<h2><?php echo $lumise->lang(ucfirst($sects)); ?></h2>
			<a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sect; ?>" class="add_new">
				<i class="fa fa-plus"></i>
				<?php echo $lumise->lang('Add New '.ucfirst($sect)); ?>
			</a>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sects; ?>" method="post">
				...
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sects; ?>" method="post" class="less">
				...
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sects; ?>" method="post" class="less">
					<select name="categories" class="art_per_page" data-action="submit" style="width:150px">
						...
						<?php
							$cates = $lumise_admin->get_categories('images');
			...
			<div class="right">
				<form action="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sects; ?>" method="post" class="less">
					...
					<input type="hidden" name="search_<?php echo $sect; ?>" value="<?php echo $lumise->lang('Search'); ?>">
					<?php $lumise->securityFrom();?>
				</form>
			</div>
		</div>
		...
		<div class="lumise_wrap_table">
			<table class="lumise_table lumise_<?php echo $sects; ?>">
				<thead>
					<tr>
					...
					<?php
						...
						foreach ($arts as $art) { ?>
							...
							<tr>
								...
								<td><a href="<?php echo $lumise->cfg->admin_url;?>lumise-page=<?php echo $sect; ?>&id=<?php if(isset($art['id'])) echo $art['id'] ?>" class="name"><?php if(isset($art['name'])) echo $art['name']; ?></a></td>
								<td style="position:relative;"><input type="number" class="lumise_set_price" data-type="<?php echo $sects; ?>" data-id="<?php if(isset($art['id'])) echo $art['id']; ?>" value="<?php if(isset($art['price'])) echo $art['price']; ?>"></td>
								<td>
									<?php
										$art['id'] = isset($art['id']) ? $art['id'] : '';
										$dt = $lumise_admin->get_category_item($art['id'], $sects);
										$dt_name = array();
								...
								<td style="width:20%; position:relative;">
									<?php
										$art['id'] = isset($art['id']) ? $art['id'] : '';
										$dt = $lumise_admin->get_tag_item($art['id'], $sects);
										$dt_name = array();
										foreach ($dt as $val) {
											$dt_name[] = $val['name'];
										}
									?>
									<input name="tags" class="tagsfield" value="<?php echo implode(',', $dt_name); ?>" data-id="<?php echo $art['id']; ?>" data-type="<?php echo $sects; ?>">
								</td>
								<td>
									<?php
								...
								<td class="lumise_featured">
									<a href="#" class="lumise_action" data-type="<?php echo $sects; ?>" data-action="switch_feature" data-status="<?php echo (isset($art['featured']) ? $art['featured'] : '0'); ?>" data-id="<?php if(isset($art['id'])) echo $art['id'] ?>">
										<?php
								...
								<td>
									<a href="#" class="lumise_action" data-type="<?php echo $sects; ?>" data-action="switch_active" data-status="<?php echo (isset($art['active']) ? $art['active'] : '0'); ?>" data-id="<?php if(isset($art['id'])) echo $art['id'] ?>">
										<?php
											if (isset($art['active'])) {
												if ($art['active'] == 1) {
		...
		<?php } else {
					if (isset($total_record[0]['total']) && $total_record[0]['total'] > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'lumise-page='.$sects.'" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else echo '<p class="no-data">'.$lumise->lang('No data. Please add '.$sect).'</p>';
			}?>
...
var nonce = "<?php echo lumise_secure::create_nonce('LUMISE_ADMIN_'.$sects) ?>",
	reader = [],
	total = 0,
	done = 0;
<?php
